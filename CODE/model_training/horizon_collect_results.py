from CODE.model_training.horizon_model_training import *
import pandas as pd
import numpy as np
import json
...
def calculate_loss_baseline_horizon(df, target, horizon):
    ...
    # take numbers starting from horizon-1, step by horizon
    i = horizon-1
    preds = []
    while i < len(df['y']) - 1:
...
def objective(trial, df, intrest):
    ...
    return mean_return  # Optuna minimizes the objective function, so we use the negative of mean_return

def optimize_parameters_with_optuna(df, intrest):
    study = optuna.create_study(direction='maximize')
    study.optimize(lambda trial: objective(trial, df, intrest), n_trials=600)  # You can adjust n_trials as needed
